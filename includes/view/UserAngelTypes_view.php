...
{
    return page_with_title(__('Confirm critter type for user'), [
        msg(),
        info(sprintf(
...
{
    return page_with_title(__('Leave critter type'), [
        msg(),
        info(sprintf(
...
function UserAngelType_add_view(AngelType $angeltype, $users_select, $user_id)
{
    ...
    return page_with_title($link . ' ' . __('Add user to critter type'), [
        msg(),
        form([
            form_info(__('Critter type'), htmlspecialchars($angeltype->name)),
