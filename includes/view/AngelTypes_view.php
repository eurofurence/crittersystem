...
function AngelType_delete_view(AngelType $angeltype)
{
    ...
    return page_with_title($link . ' ' . sprintf(__('Delete critter type %s'), htmlspecialchars($angeltype->name)), [
        info(sprintf(__('Do you want to delete critter type %s?'), $angeltype->name), true),
        form([
            buttons([
...
function AngelType_view_buttons(
    ...
    if (is_null($user_angeltype)) {
        ...
        if ($angeltype->restricted && !$user_angeltype->confirm_user_id) {
            error(sprintf(
                __('You are unconfirmed for this critter type. Please go to the introduction for %s to get confirmed.'),
                $angeltype->name
            ));
        }
        $buttons[] = button(
